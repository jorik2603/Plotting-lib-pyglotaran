...
class ShoveExplicitFile:
    """
    Abstract class representing either a wavelength-explicit file.
    """
    def __init__(self, filepath: str | None, dataset: xr.DataArray | None = None):
        self._file_data_format = None
        self._data = []
        self._times = []
        self._spectral_indices = []  # Fixed typo in initial variable definition to match usage
        self._label = ""
        self._comment = ""
    ...
    def read(self, prepare: bool = True) -> xr.Dataset:        
        ...
        # Read data from file - changed separator from ',' to '\t' to read tab-separated files
        wv_axis = pd.read_csv(self._file, sep='\t', nrows=1, header=None).values
        data_arr = pd.read_csv(self._file, skiprows=1, sep='\t', header=None).values
        
        wv_axis[np.isnan(wv_axis)] = 0
        wv_axis[np.isinf(wv_axis)] = 0
        data_arr[np.isnan(data_arr)] = 0
        data_arr[np.isinf(data_arr)] = 0
        
        # Fixed bug: Reassigning the sorted array back to data_arr
        data_arr = data_arr[data_arr[:,0].argsort()]
        
        # Remove duplicate time points
        times, idx = np.unique(data_arr[:,0], return_index=True)
...
class WavelengthExplicitFile(ShoveExplicitFile):
    """
...
@register_data_io(["dat", "Shove"])
class ShoveDataIO(DataIoInterface):
