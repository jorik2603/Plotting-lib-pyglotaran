...
class DelayOscillationParameterIssue(ItemIssue):
    def __init__(self, label: str, len_labels: int, len_frequencies: int, len_rates: int, len_delay: int,):
        self.label = label
    ...
    def to_string(self) -> str:
        return (
            ...
            f"rates ({self.len_rates}), and delay ({self.len_delay}) does not match for damped oscillation "
            f"megacomplex '{self.label}'."
        )


def validate_delay_oscillation_parameter(
    labels: list[str],
    damped_oscillation: DelayDampedOscillationMegacomplex,
    model: Model,
    parameters: Parameters | None,
) -> list[ItemIssue]:
    issues = []

    len_labels, len_frequencies, len_rates, len_delay = (
        len(damped_oscillation.labels),
        len(damped_oscillation.frequencies),
        len(damped_oscillation.rates),
        len(damped_oscillation.delay),
    )

    if len({len_labels, len_frequencies, len_rates, len_delay}) > 1:
        issues.append(
            DelayOscillationParameterIssue(
                damped_oscillation.label, len_labels, len_frequencies, len_rates, len_delay
            )
        )
...
class DelayDampedOscillationMegacomplex(Megacomplex):
    dimension: str = "time"
    type: str = "delayed-damped-oscillation"
    labels: list[str] = attribute(validator=validate_delay_oscillation_parameter)
    frequencies: list[ParameterType]
    rates: list[ParameterType]
    delay: list[ParameterType]
            ...
            if index_dependent(dataset_model):
                for i in range(global_axis.size):
                    calculate_damped_oscillation_matrix_gaussian_irf_on_index(
                        matrix[i], frequencies, rates, delay, irf, i, global_axis, model_axis
                    )
            else:
                calculate_damped_oscillation_matrix_gaussian_irf_on_index(
                    matrix, frequencies, rates, delay, irf, None, global_axis, model_axis
                )
    ...
    def finalize_data(
        ...
        unique = len([m for m in megacomplexes if isinstance(m, DelayDampedOscillationMegacomplex)]) < 2
        ...
        dataset.coords[f"{prefix}_delay"] = (prefix, self.delay)
...
def calculate_damped_oscillation_matrix_no_irf(matrix, frequencies, rates, axis):
    "Delay requires irf to work"
    idx = 0
    print("DelayDampedOscillationMegacomplex requires an irf to work.")
...
def calculate_damped_oscillation_matrix_gaussian_irf(
    ...
    delay : np.ndarray
        an array of delays moving the starting time of the oscillation
    model_axis : np.ndarray
        the model axis (time)
    center : float
    ...
    """
    shifted_axis = model_axis - center - shift - delay
